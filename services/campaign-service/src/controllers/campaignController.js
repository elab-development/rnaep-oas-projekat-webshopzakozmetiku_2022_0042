...
const getAllCampaigns = async (req, res) => {
  try {
    ...
    await pool.query(
      `
      UPDATE campaigns 
      SET is_active = (start_date <= $1 AND end_date >= $1)
    `,
      [now],
    );
...
const addCampaign = async (req, res) => {
  try {
    ...
    if (!name || !start_date || !end_date) {
      return res
        .status(400)
        .json({ message: "Naziv, datum pocetka i zavrsetka su obavezni" });
    }
...
const addPromoCode = async (req, res) => {
  try {
    ...
    if (!campaign_id || !code || !discount_value) {
      return res
        .status(400)
        .json({ message: "campaign_id, code i discount_value su obavezni" });
    }
